...
public class PartitionManager extends AbstractListenerManager<PartitionEvent, PartitionEventListener>
        ...
        if (upgradeService.isUpgrading() && upgradeService.isLocalUpgraded()) {
            currentClusterMetadata.get()
                    ...
                    .forEach(partition -> {
                        ...
                        inactivePartitions.put(partition.getId(), new DefaultStoragePartition(
                                partition,
                                clusterCommunicator,
                                clusterService));
            ...
            currentClusterMetadata.get()
                    .getPartitions()
                    .forEach(partition -> activePartitions.put(partition.getId(), new DefaultStoragePartition(
                            partition,
                            clusterCommunicator,
                            clusterService)));
            openFuture = CompletableFuture.allOf(activePartitions.values().stream()
     ...
     */
    private List<NodeId> getSortedNodes() {
        return membershipService.getLocalGroup()
                .members()
                .stream()
                .map(Member::nodeId)
                .sorted(Comparator.comparing(clusterService::getLastUpdated))
                .collect(Collectors.toList());
    }
    ...
    private void processUpgradeComplete(Upgrade upgrade) {
        if (!inactivePartitions.isEmpty()) {
            Collection<CompletableFuture<Void>> closeFutures = inactivePartitions.values().stream()
